    ...
    const computeReport = () => {
        ...
        const perStack = stacks.map((stack) => {
            ...
            indices.forEach((i) => {
                ...
                topicMap[t].total += 1;
                if (responses[i] && responses[i].status === "correct") topicMap[t].correct += 1;
            });

            const topics = Object.entries(topicMap).map(([t, v]) => ({ topic: t, total: v.total, correct: v.correct, percent: v.total ? Math.round((v.correct / v.total) * 100) : 0 }));
